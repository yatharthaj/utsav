...
@section('content')
    <div class="row">
            ...
            <div class="table-responsive">
                <table id="datatable1" class="table table-striped table-hover">
                    ...
                    @foreach($places as $place)
                        <tr class="item{{$place->id}}">
                            <td><h4>{{$loop->iteration}}</h4></td>
                            <td><h4>{{$place->name}}</h4></td>
                            <td>
                                <button type="submit" class="btn ink-reaction btn-floating-action btn-sm btn-info " id="edit-modal" style="margin-right: 10px;"
                                        data-id="{{$place->id}}" data-name="{{$place->name}}" data-toggle="modal"
                                        data-target="#formModal">
                                    <i class="fa fa-pencil" aria-hidden="true"></i>
                                </button>
                                <button type="button" class="btn ink-reaction btn-floating-action btn-sm  btn-danger" data-id="{{$place->id}}" data-toggle="modal"
        ...
        <div class="col-lg-4">
            <form class="form" method="POST" action="{{ route('locations.store') }}">
                {{ csrf_field() }}
                <div class="card">
                    <div class="card-head style-primary">
                        <header>Add new location</header>
                    </div>
